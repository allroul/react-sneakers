          ...
          <div className='cart__body' ref={cartBodyRef} onClick={onCloseCart}>
            <div className={`cart__content ${classTransitions}`}>
              <h3 className='cart__title'>
                Корзина
                <img src='img/close-cart.png' alt='close cart' onClick={() => dispatch(changeCartStatus(false))} />
              </h3>
              {cartItems.length > 0 ? 
                (cartList) : orderStatus 
                ? <CartInfo title='Заказ оформлен!' image='img/order-completed.jpg' text={`Ваш заказ #${orderNumber} скоро будет передан курьерской доставке`} completed/> 
                : <CartInfo title='Корзина пустая' image='img/cart-empty.png' text='Добавьте хотя бы одну пару кроссовок, чтобы сделать заказ.'/>}      
